...
## 禁止写__CTI_BOUNDARY_*这种boundary 标记标记功能，因为这是弊大于利的事情，虽然this_command_output 的“粘连/误判”概率会上升，只能靠 cursor/since + noEcho + prompt 检测兜底，但是没办法。。。

## 修改代码后，请务必以'd:\CodeRelated\test'为工作目录调用MCP进行一些命令的测试来检查刚才修改的代码是否正确，禁止在其他地方测试。